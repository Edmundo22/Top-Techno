...
- **Polígono**: `TerraDrawPolygonMode` — botão "Desenhar polígono" → `setMode('polygon')`; fecha clicando no 1º ponto ou com duplo-clique. UX do desenho (espelha o antigo DrawingManager):
  - **Sem preenchimento enquanto desenha**: `fillOpacity`/`coordinatePointOpacity` são funções de `feature.properties.currentlyDrawing` — durante o desenho mostra só os vértices (`showCoordinatePoints: true`) e o contorno; o fill (`0.4`) só aparece depois de fechar o anel.
  - **Apagar último ponto**: botão chama `polygonMode.undo()` (guardado por `undoSize() > 0`) — `polygonModeRef` guarda a instância do mode. Remove o último vértice colocado.
  - **Editar depois de fechado**: ao concluir (`finish` com `action: 'draw'`, `mode: 'polygon'`) faz `setMode('select')` + `draw.selectFeature(id)`, já deixando o polígono selecionado para arrastar vértices/midpoints. O `TerraDrawSelectMode` é o modo padrão (sem listeners manuais).
