...
const linkBase = 'group flex h-10 items-center gap-3 rounded-lg px-3 text-sm transition-colors';
...
export function Sidebar({ mobileOpen, onCloseMobile }: SidebarProps) {
  ...
  const renderBrand = (showText: boolean) => (
    <div className="flex h-16 items-center gap-3 border-b border-white/10 px-4">
      ...
      {showText && (
        <div className="flex min-w-0 flex-col leading-tight">
          <span className="truncate text-sm font-semibold">TOP TECHNO</span>
          <span className="truncate text-[11px] text-white/60">Sistema interno</span>
        </div>
      )}
    </div>
  );

  const renderNav = (navCollapsed: boolean, onNavigate?: () => void) => (
  ...
  return (
    <>
      {/* Desktop (lg+): aside fixa, com colapso */}
      <aside
        className={`hidden h-full shrink-0 flex-col bg-brand-ink text-white transition-[width] duration-200 lg:flex ${
          collapsed ? 'w-16' : 'w-60'
        }`}
      >
      ...
      {/* Mobile (< lg): drawer off-canvas */}
      <div className={`fixed inset-0 z-40 lg:hidden ${mobileOpen ? '' : 'pointer-events-none'}`}>
        <div
          className={`absolute inset-0 bg-brand-ink/50 transition-opacity duration-200 ${
        ...
        />
        <aside
          className={`absolute inset-y-0 left-0 flex w-64 max-w-[82vw] flex-col bg-brand-ink text-white shadow-float transition-transform duration-200 ${
        ...
        >
          <div className="relative">
            ...
            <button
              ...
              className="absolute right-3 top-1/2 grid h-8 w-8 -translate-y-1/2 place-items-center rounded-lg text-white/70 transition-colors hover:bg-white/10 hover:text-white"
            >
